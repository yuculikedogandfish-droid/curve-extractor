...
| **v1.4.24** | `2e50c5a` | 分叉沿原图接到主枝/根系；「分叉着生」可调 | 可回滚 |
| **v1.4.25** | `fff1e1e` | **网页当前**：走势轴向可调；分级按原图颜色 | **Pages 线上** |
| **v1.4.26** | DCC 包 | **Blender / Houdini 独立插件**：本机提曲线，不走公网网页 | **发给特效师** |
...
### v1.4.25 `fff1e1e`（网页当前）
- **走势轴向**滑条：工具曾默认「从画面底部往上长」，横幅光带会被截成一段段。往左=横向连续环绕，往右=竖向根系，中间按图片长宽自动判断。
- 一/二/三级改为 **按原图颜色分组**（色组一/二/三），不再按粗细上色、不再用粗细干扰预览线宽。
...
| 走势轴向 | `getGrowthAxis()`、`#growthAxis` |
| 颜色分级 | `classifyCurves()`、`sampleCurveColor()`、`#chkShowL1` 等 |
